export default class PipelineArea {
  constructor() {
    ...
    this.changePipelineStatusOption = this.container.$('button#this-pipeline-toggle')
    this.pipelineStatusSecondRelativeOption = this.container.$('div.student-status_status button:nth-child(2)')
    ...
    this.addToAnotherPipelineFirstOption = this.addToAnotherPipelineDropdown.element(by.repeater('item in items').row(0))
    this.pipelineStatusesOpenedMenu = this.container.$('ul.open')
    this.pipelineStatusesSecondOption = this.pipelineStatusesOpenedMenu.$('li:nth-child(2) a')
    this.lastCheckboxList = this.container.all(by.css('.student-status_list')).last()
    this.decidingStatusFirstCheckbox = this.lastCheckboxList.all(by.css('input[type="checkbox"]')).get(0)
    this.decidingStatusSecondCheckbox = this.lastCheckboxList.all(by.css('input[type="checkbox"]')).get(1)
    this.decidingStatusThirdCheckbox = this.lastCheckboxList.all(by.css('input[type="checkbox"]')).get(2)
  }
